...
public class RunEndHandler {
    ...
    public static void checkAndGrantMasterAchievement(ServerPlayer player, CarryoverData carryover, ModConfig config) {
        ...
        if (allEarned) {
            ...
            CarryoverStorage.save(carryover);
            if (player.getServer() != null && (masterAdv = player.getServer().getAdvancements().get(ResourceLocation.fromNamespaceAndPath((String)"timeattackroguelike", (String)"master"))) != null && !(tracker = player.getAdvancements()).getOrStartProgress(masterAdv).isDone()) {
            ...
            TimeAttackRoguelike.LOGGER.info("Player {} earned Master achievement for kit: {}", player.getName().getString(), kitName);
        } else {
            CarryoverStorage.save(carryover);
        }
    }
