...
extends AbstractContainerMenu {
    ...
    public CarryoverSelectScreenHandler(int syncId, Inventory playerInventory, Container inventory, double points, Map<Integer, String> slotSources, String kitName) {
        super((MenuType)TimeAttackRoguelike.CARRYOVER_SCREEN_HANDLER.get(), syncId);
        int j;
        int i;
        this.points = points;
        ...
        if (!this.player.level().isClientSide()) {
                ...
                if (data.isArmorSlotUnlocked(i)) {
                    ...
                    if (!stack.isEmpty()) {
                        try {
                            ...
                            TimeAttackRoguelike.LOGGER.error("Failed to save armor carryover item", (Throwable)e);
                            armorItems.add("");
                        }
                        continue;
                    }
            ...
            if (data.isAccessorySlotUnlocked()) {
                ...
                if (!stack.isEmpty()) {
                    try {
                        ...
                        TimeAttackRoguelike.LOGGER.error("Failed to save accessory carryover item", (Throwable)e);
            ...
            if (!freeStack.isEmpty()) {
                try {
                    ...
                    TimeAttackRoguelike.LOGGER.error("Failed to save free carryover item", (Throwable)e);
                ...
                if (i < unlocked) {
                    ...
                    if (!stack.isEmpty()) {
                        try {
                            ...
                            TimeAttackRoguelike.LOGGER.error("Failed to save carryover item", (Throwable)e);
                            carryoverItems.add("");
                        }
                        continue;
                    }
